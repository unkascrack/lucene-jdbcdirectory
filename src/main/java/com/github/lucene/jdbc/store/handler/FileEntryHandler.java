...
package com.github.lucene.jdbc.store.handler;
...
/**
 * A file entry handler acts as a delegate to the {@link JdbcDirectory} for all
 * "file" level operations. Allows the {@link JdbcDirectory} to be abstracted
 * from any specific implementation details regarding a file entry, and have
 * several different file entries for different files or files groups.
 *
 * @author kimchy
 * @see org.apache.lucene.store.jdbc.JdbcDirectorySettings#registerFileEntrySettings(String,
 *      org.apache.lucene.store.jdbc.JdbcFileEntrySettings)
 */
public interface FileEntryHandler {

    /**
     * Called after the entry is created (during the {@link JdbcDirectory}
     ...
     * @return <code>true</code> of the file exists, <code>false</code> if it
     *         does not.
     ...
     * that maps to the actual file entry handler.
     *
     * @param from
