...
import com.github.lucene.store.jdbc.handler.MarkDeleteFileEntryHandler;
import com.github.lucene.store.jdbc.index.FetchOnBufferReadJdbcIndexInput;
 ...
 * @author kimchy
 */
public class JdbcFileEntrySettingsTest extends TestCase {

    public void testDefaultSettings() throws Exception {
        final JdbcFileEntrySettings settings = new JdbcFileEntrySettings();

        assertEquals(3, settings.getProperties().size());
        assertEquals(MarkDeleteFileEntryHandler.class,
                settings.getSettingAsClass(JdbcFileEntrySettings.FILE_ENTRY_HANDLER_TYPE, null));
        assertEquals(FetchOnBufferReadJdbcIndexInput.class,
                settings.getSettingAsClass(JdbcFileEntrySettings.INDEX_INPUT_TYPE_SETTING, null));
        assertEquals(RAMAndFileJdbcIndexOutput.class,
                settings.getSettingAsClass(JdbcFileEntrySettings.INDEX_OUTPUT_TYPE_SETTING, null));
    }
